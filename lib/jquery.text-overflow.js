            ...
            if (!hasTextOverflow || o.force) {
                return this.each(function () {
                    var element = $(this),
                        ...
                        checkWidth = originalElement.css('white-space') == 'nowrap',
                        reflow = function () {
                    ...
                    if ((checkWidth && clone.width() > originalWidth) || (!checkWidth && clone.height() > originalHeight)) {
                        while (low < high) {
                            // Find the best match by binary search
                            mid = Math.floor(low + ((high - low) / 2));
							clone.empty().append(domSplit(originalElement.get(0), mid, o)).append(o.str);
                            if ((checkWidth && clone.width() <= originalWidth)
                                || (!checkWidth && clone.height() <= originalHeight)) {
                                low = mid + 1;
                            } else {
                                high = mid;
                            }
